...
# ─────────────────────────────────────────────
INDUSTRY_BENCHMARK = 39.6

BENCHMARKS = [
    ("Overall Turnover Rate",       "23.81%", "39.6%", "under"),
    ("Mid-Tenure Attrition",        "40.69%", "39.6%", "over"),
    ("2-Project Assignment",        "65.62%", "39.6%", "over"),
    ("6-Project Assignment",        "55.79%", "39.6%", "over"),
    ("7-Project Assignment",        "100.0%", "39.6%", "over"),
    ("Low Workload Turnover",       "36.35%", "39.6%", "under"),
    ("High Workload Turnover",      "30.72%", "39.6%", "under"),
    ("Moderate Workload Turnover",  "4.35%",  "39.6%", "under"),
    ("High Salary Turnover",        "6.63%",  "39.6%", "under"),
    ("Promoted Employee Turnover",  "5.96%",  "39.6%", "under"),
]
...
with st.sidebar:
    ...
    <span style='color:#2a5572;'>Benchmark: BLS JOLTS Manufacturing<br>39.6% annualized (3.3%/mo)</span>
...
<div class="kpi-banner">
    ...
    <div class="banner-divider"></div>
    <div class="banner-item">
        <span class="banner-label">JOLTS Benchmark</span>
        <span class="banner-value blue">39.6%</span>
    </div>
    <span class="banner-source">BLS JOLTS Manufacturing · Annualized · Updated Feb 2026</span>
</div>
""", unsafe_allow_html=True)
...
if page == "Executive Summary":
    ...
    with c3: st.markdown(kpi("Turnover Rate",    f"{turnover}%",  "JOLTS Mfg. benchmark: 39.6%", clr),      unsafe_allow_html=True)
    ...
    📌 <strong>Key Finding:</strong> At <strong>{turnover}%</strong>, Alpha's overall turnover is
    <strong>below</strong> the U.S. manufacturing JOLTS benchmark of <strong>39.6%</strong> — indicating
    general workforce stability at the aggregate level. However, turnover is heavily concentrated in
    specific segments. The <strong>{top_dept}</strong> department leads at <strong>{top_rate}%</strong>.
    Mid-tenure employees (years 4–6) and extreme project assignments (2 or 7 projects) significantly
    exceed the benchmark and represent the primary structural risk areas.
    </div>""", unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.markdown('<div class="section-sub">Ranked highest to lowest · JOLTS benchmark at 39.6%</div>', unsafe_allow_html=True)
        ...
        colors = [RED if r > INDUSTRY_BENCHMARK else (ORANGE if r > 20 else PALETTE[2])
                  for r in dept_df["Turnover Rate (%)"]]
        ...
        fig.add_vline(x=INDUSTRY_BENCHMARK, line_dash="dash", line_color="#e74c3c",
                      annotation_text="JOLTS 39.6%", annotation_font_size=9,
                      annotation_position="top right", annotation_font_color="#c0392b")
        fig.update_layout(yaxis=dict(showgrid=False), xaxis=dict(range=[0, 55]))
        st.plotly_chart(fig, use_container_width=True)

    with col2:
        st.markdown('<div class="section-title">Turnover Rate by Salary Band</div>', unsafe_allow_html=True)
        st.markdown('<div class="section-sub">Compensation level vs. attrition · JOLTS benchmark at 39.6%</div>', unsafe_allow_html=True)
        ...
        fig2 = go.Figure(go.Bar(
            x=sal_df["salary"], y=sal_df["Turnover Rate (%)"],
            marker_color=[RED, ORANGE, GREEN],
            text=sal_df["Turnover Rate (%)"].apply(lambda x: f"{x}%"),
            textposition="outside", width=0.45))
        fig2 = chart_layout(fig2, 360)
        fig2.add_hline(y=INDUSTRY_BENCHMARK, line_dash="dash", line_color="#e74c3c",
                       annotation_text="JOLTS 39.6%", annotation_font_size=9,
                       annotation_position="top right", annotation_font_color="#c0392b")
        fig2.update_layout(yaxis=dict(range=[0, 50]))
    ...
    st.markdown('<div class="section-sub">Alpha Manufacturing vs. U.S. manufacturing sector (BLS JOLTS ~39.6% annualized) · Source: Meridian Industry Benchmark Report</div>', unsafe_allow_html=True)
    ...
    with col4:
        ...
        bar_colors = [RED if a > INDUSTRY_BENCHMARK else GREEN for a in bm_alpha]
        fig_bm = go.Figure()
        fig_bm.add_trace(go.Bar(name="Alpha Manufacturing", x=bm_labels, y=bm_alpha,
                                marker_color=bar_colors, opacity=0.9))
        fig_bm.add_trace(go.Scatter(name="JOLTS Benchmark (39.6%)",
                                    x=bm_labels, y=[INDUSTRY_BENCHMARK]*len(bm_labels),
                                    mode="lines", line=dict(color="#e74c3c", dash="dash", width=2)))
        fig_bm = chart_layout(fig_bm, 360)
...
# ══════════════════════════════════════════════
    ...
    st.markdown(f"""<div class="insight-box">
    📌 <strong>Analyst Note:</strong> While Alpha's overall turnover (23.81%) sits below the JOLTS benchmark of 39.6%,
    several segments significantly exceed it — mid-tenure employees at <strong>40.69%</strong>, 2-project employees
    at <strong>65.62%</strong>, and 7-project employees at <strong>100%</strong>. Attrition peaks sharply at
    <strong>year 5 (56.6%)</strong> before dropping to zero for employees who reach 7+ years, suggesting a clear
    critical retention window between years 3–6.
    </div>""", unsafe_allow_html=True)
    ...
    with col1:
        ...
        st.markdown('<div class="section-sub">Exact year-by-year turnover · peak at year 5 (56.6%) · JOLTS benchmark at 39.6%</div>', unsafe_allow_html=True)
        ...
        fig_ty.add_trace(go.Bar(
            x=tenure_yr["time_spend_company"],
            y=tenure_yr["Turnover Rate (%)"],
    ...
    with col2:
        ...
        st.markdown('<div class="section-sub">Monthly hours bucketed into tiers · JOLTS benchmark at 39.6%</div>', unsafe_allow_html=True)
        ...
        fig5 = go.Figure(go.Bar(
            x=wl_df["workload_level"], y=wl_df["Turnover Rate (%)"],
            marker_color=[ORANGE, GREEN, RED],
            text=wl_df["Turnover Rate (%)"].apply(lambda x: f"{x}%"),
            textposition="outside", width=0.4))
        fig5 = chart_layout(fig5, 320)
        fig5.add_hline(y=INDUSTRY_BENCHMARK, line_dash="dash", line_color="#e74c3c",
                       annotation_text="JOLTS 39.6%", annotation_font_size=9,
                       annotation_position="top right", annotation_font_color="#c0392b")
        fig5.update_layout(yaxis=dict(range=[0, 55]))
    ...
    with col3:
        ...
        pcolors = [RED if r > INDUSTRY_BENCHMARK else (ORANGE if r > 20 else GREEN)
                   for r in proj_df["Turnover Rate (%)"]]
        fig6 = go.Figure(go.Bar(
            x=proj_df["number_project"], y=proj_df["Turnover Rate (%)"],
    ...
    with col4:
        ...
        fig7.add_hline(y=INDUSTRY_BENCHMARK, line_dash="dash", line_color="#e74c3c",
                       annotation_text="JOLTS 39.6%", annotation_font_size=9,
                       annotation_position="top right", annotation_font_color="#c0392b")
        fig7.update_layout(yaxis=dict(range=[0, 50]))
