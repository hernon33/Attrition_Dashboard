...
<div class="kpi-banner">
    ...
    <span class="banner-source">Baseline: 2024–25 Mfg. Industry Surveys · Segment benchmarks: est. assumptions · Data refreshed: {datetime.now().strftime("%b %d, %Y")}</span>
</div>
""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown('<div class="section-title">Satisfaction Score Distribution — Left vs. Active</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">Distribution of satisfaction levels · leavers cluster at both extremes · not just the bottom</div>', unsafe_allow_html=True)

    col_s1, col_s2 = st.columns(2)
    with col_s1:
        fig_sdist = go.Figure()
        fig_sdist.add_trace(go.Histogram(
            x=fdf[fdf["left"]==1]["satisfaction_level"],
            name="Left", marker_color=RED, opacity=0.75,
            nbinsx=25, histnorm="percent"))
        fig_sdist.add_trace(go.Histogram(
            x=fdf[fdf["left"]==0]["satisfaction_level"],
            name="Active", marker_color=PALETTE[0], opacity=0.75,
            nbinsx=25, histnorm="percent"))
        fig_sdist.update_layout(barmode="overlay")
        fig_sdist = chart_layout(fig_sdist, 320)
        fig_sdist.update_layout(
            xaxis=dict(title="Satisfaction Level (0–1)"),
            yaxis=dict(title="% of Group"),
            legend=dict(orientation="h", y=1.1))
        st.plotly_chart(fig_sdist, use_container_width=True)
        st.markdown("""<div class="data-note">
        <strong>📎 Key Insight:</strong> Leavers show a <strong>bimodal distribution</strong> —
        clustering at both very low satisfaction (&lt;0.35) AND surprisingly high satisfaction
        (≥0.65). The high-satisfaction leavers are likely overloaded high performers leaving due
        to burnout, not dissatisfaction. This means a single satisfaction intervention will not
        address both leaver types.
        </div>""", unsafe_allow_html=True)

    # ── Satisfaction vs Tenure ──────────────────────────────────────────
    with col_s2:
        st.markdown('<div class="section-title" style="margin-top:0;">Satisfaction Score by Tenure Year</div>', unsafe_allow_html=True)
        st.markdown('<div class="section-sub">Avg satisfaction drops sharply at year 4 — directly explains the mid-tenure attrition spike</div>', unsafe_allow_html=True)
        sat_tenure = fdf.groupby("time_spend_company").agg(
            avg_sat=("satisfaction_level", "mean"),
            turnover=("left", "mean")
        ).reset_index()
        sat_tenure["avg_sat"]  = sat_tenure["avg_sat"].round(3)
        sat_tenure["turnover"] = (sat_tenure["turnover"] * 100).round(1)

        fig_st = go.Figure()
        sat_colors_t = [RED if s < 0.50 else (ORANGE if s < 0.60 else GREEN)
                        for s in sat_tenure["avg_sat"]]
        fig_st.add_trace(go.Bar(
            x=sat_tenure["time_spend_company"], y=sat_tenure["avg_sat"],
            marker_color=sat_colors_t,
            text=sat_tenure["avg_sat"].apply(lambda x: f"{x:.3f}"),
            textposition="inside",
            textfont=dict(color="white", size=9),
            name="Avg Satisfaction", yaxis="y1"))
        fig_st.add_trace(go.Scatter(
            x=sat_tenure["time_spend_company"], y=sat_tenure["turnover"],
            name="Turnover Rate %", mode="lines+markers",
            line=dict(color=RED, width=2, dash="dot"),
            marker=dict(size=7, color=RED),
            yaxis="y2"))
        fig_st = chart_layout(fig_st, 320)
        fig_st.update_layout(
            xaxis=dict(title="Years at Company", dtick=1),
            yaxis=dict(title="Avg Satisfaction", range=[0, 1], showgrid=True),
        ...
        st.markdown("""<div class="data-note">
        <strong>📎 Key Insight:</strong> Satisfaction drops to <strong>0.468</strong> at year 4 —
        the lowest point across all tenure years — while turnover simultaneously spikes to 34.8%.
        This is direct evidence that career stagnation, not external factors, drives the
        mid-tenure crisis.
        </div>""", unsafe_allow_html=True)
    ...
    # ── Correlation Matrix ────────────────────────────────────────────────
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown('<div class="section-title">Correlation Matrix — All Key Variables</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">Pearson correlation · red = positive correlation with attrition · blue = negative · darker = stronger</div>', unsafe_allow_html=True)

    corr_df = fdf.copy()
    corr_df["salary_num"] = corr_df["salary"].map({"low":0,"medium":1,"high":2})
    corr_cols   = ["left","satisfaction_level","last_evaluation","number_project",
                   "average_montly_hours","time_spend_company","Work_accident",
                   "promotion_last_5years","salary_num"]
    corr_labels = ["Left","Satisfaction","Last Eval","# Projects",
                   "Avg Hours","Tenure","Work Accident","Promoted","Salary"]
    ...
    st.markdown("""<div class="data-note">
    <strong>📎 Key Insight:</strong> <strong>Satisfaction level</strong> has the strongest negative
    correlation with attrition (r = -0.39) — meaning lower satisfaction strongly predicts leaving.
    <strong>Salary</strong> (r = -0.16) and <strong>Work Accident</strong> (r = -0.15) also show
    meaningful negative correlations. <strong>Tenure</strong> (r = 0.14) and
    <strong>Monthly Hours</strong> (r = 0.07) show positive correlations — longer tenure and
    higher hours are associated with higher attrition risk.
    </div>""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════
    ...
    # ── Employee Persona Cards ──────────────────────────────────────────
    st.markdown('<div class="section-title">At-Risk Employee Personas</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">Three distinct leaver profiles identified from the data · each requires a different retention strategy</div>', unsafe_allow_html=True)

    p1, p2, p3 = st.columns(3)

    with p1:
        st.markdown(f"""
        <div style="background:#ffffff; border-radius:10px; padding:1.3rem; height:100%;
             box-shadow:0 2px 12px rgba(0,0,0,0.06); border-top:4px solid {RED};">
            <div style="font-size:2rem; margin-bottom:0.4rem;">🔥</div>
            <div style="font-family:'DM Serif Display',serif; font-size:1rem;
                 color:#1a252f; margin-bottom:0.3rem;">The Burned-Out Performer</div>
            <div style="font-size:0.7rem; font-weight:700; letter-spacing:0.08em;
                 text-transform:uppercase; color:{RED}; margin-bottom:0.8rem;">
                878 employees · 24.6% of leavers
            </div>
            <div style="font-size:0.8rem; color:#2c3e50; line-height:1.7;">
                <b>Profile:</b> High evaluation scores, 6–7 projects, 273 avg hrs/month,
                4 years tenure on average.<br><br>
                <b>Why they leave:</b> Not due to dissatisfaction with the company —
                they are high performers being overloaded until they break.
                Avg satisfaction: <b>0.12</b> at point of departure.<br><br>
                <b>What they need:</b> Workload caps, recognition, and a career
                path that doesn't require burning out to advance.
            </div>
        </div>""", unsafe_allow_html=True)
    ...
    with p2:
        st.markdown(f"""
        <div style="background:#ffffff; border-radius:10px; padding:1.3rem; height:100%;
             box-shadow:0 2px 12px rgba(0,0,0,0.06); border-top:4px solid {ORANGE};">
            <div style="font-size:2rem; margin-bottom:0.4rem;">😶</div>
            <div style="font-family:'DM Serif Display',serif; font-size:1rem;
                 color:#1a252f; margin-bottom:0.3rem;">The Disengaged Underutilized</div>
            <div style="font-size:0.7rem; font-weight:700; letter-spacing:0.08em;
                 text-transform:uppercase; color:{ORANGE}; margin-bottom:0.8rem;">
                1,534 employees · 43% of leavers
            </div>
            <div style="font-size:0.8rem; color:#2c3e50; line-height:1.7;">
                <b>Profile:</b> 2 projects or fewer, 145 avg hrs/month, 3 years
                tenure, low satisfaction (avg 0.41).<br><br>
                <b>Why they leave:</b> Underutilization creates disengagement.
                These employees feel their skills are wasted and quietly disengage
                before exiting — the classic flight risk.<br><br>
                <b>What they need:</b> Meaningful work assignments, visible
                contribution to team goals, and manager engagement.
            </div>
        </div>""", unsafe_allow_html=True)
    ...
    with p3:
        st.markdown(f"""
        <div style="background:#ffffff; border-radius:10px; padding:1.3rem; height:100%;
             box-shadow:0 2px 12px rgba(0,0,0,0.06); border-top:4px solid #6c3483;">
            <div style="font-size:2rem; margin-bottom:0.4rem;">⏳</div>
            <div style="font-family:'DM Serif Display',serif; font-size:1rem;
                 color:#1a252f; margin-bottom:0.3rem;">The Stagnant Mid-Tenure Employee</div>
            <div style="font-size:0.7rem; font-weight:700; letter-spacing:0.08em;
                 text-transform:uppercase; color:#6c3483; margin-bottom:0.8rem;">
                1,928 employees · 54% of leavers
            </div>
            <div style="font-size:0.8rem; color:#2c3e50; line-height:1.7;">
                <b>Profile:</b> 4–6 years tenure, no promotion, 60% on low salary,
                257 avg hrs/month, satisfaction avg 0.46.<br><br>
                <b>Why they leave:</b> They've invested years but see no path forward.
                Satisfaction drops to its lowest point at year 4 precisely because
                advancement expectations go unmet.<br><br>
                <b>What they need:</b> A visible promotion pathway, salary progression,
                and a clear answer to "what does my next 3 years look like here?"
            </div>
        </div>""", unsafe_allow_html=True)
    ...
    st.markdown("<hr>", unsafe_allow_html=True)
    st.markdown('<div class="section-title">Cost of Attrition Calculator</div>', unsafe_allow_html=True)
    st.markdown('<div class="section-sub">Estimate the financial impact of current attrition · adjust inputs to model different scenarios</div>', unsafe_allow_html=True)

    calc1, calc2, calc3 = st.columns(3)
    with calc1:
        avg_salary    = st.number_input("Average Annual Salary ($)", min_value=20000,
                                         max_value=500000, value=55000, step=5000)
    with calc2:
        cost_pct      = st.slider("Replacement Cost (% of salary)", min_value=10,
                                   max_value=200, value=50,
                                   help="SHRM estimates 50–200% of salary. Conservative = 50%.")
    with calc3:
        target_reduction = st.slider("Target Turnover Reduction (pp)", min_value=1,
                                      max_value=15, value=5,
                                      help="How many percentage points you aim to reduce turnover by.")

    cost_per_hire      = avg_salary * (cost_pct / 100)
    current_departures = left_ct
    current_cost       = current_departures * cost_per_hire
    retained_employees = round(total * (target_reduction / 100))
    saved_cost         = retained_employees * cost_per_hire

    cc1, cc2, cc3, cc4 = st.columns(4)
    with cc1:
        st.markdown(kpi("Cost Per Departure", f"${cost_per_hire:,.0f}",
                        f"{cost_pct}% of ${avg_salary:,}", "danger"), unsafe_allow_html=True)
    with cc2:
        st.markdown(kpi("Current Annual Cost", f"${current_cost:,.0f}",
                        f"{current_departures:,} departures × ${cost_per_hire:,.0f}", "danger"), unsafe_allow_html=True)
    with cc3:
        st.markdown(kpi("Employees Retained", f"{retained_employees:,}",
                        f"At {target_reduction}pp turnover reduction", "success"), unsafe_allow_html=True)
    with cc4:
        st.markdown(kpi("Estimated Annual Savings", f"${saved_cost:,.0f}",
                        f"By reducing turnover {target_reduction}pp", "success"), unsafe_allow_html=True)

    st.markdown(f"""<div class="insight-box green">
    💰 <strong>Business Case:</strong> At an average salary of <strong>${avg_salary:,}</strong> and
    a replacement cost of <strong>{cost_pct}% of salary</strong>, Alpha's current attrition of
    <strong>{current_departures:,} employees</strong> costs an estimated
    <strong>${current_cost:,.0f} per year</strong>. Reducing turnover by just
    <strong>{target_reduction} percentage points</strong> would retain approximately
    <strong>{retained_employees:,} employees</strong> and save an estimated
    <strong>${saved_cost:,.0f} annually</strong> — a clear ROI for retention investment.
    </div>""", unsafe_allow_html=True)
